    ...
    <div className={`rounded-xl border p-4 ${
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className={`text-[14px] font-medium ${tone === "danger" ? "text-rose-200" : "text-amber-300"}`}>
          ...
          <ul className="mt-1.5 space-y-1">
            {allergies.slice(0, 4).map((a, i) => (
              <li key={i} className="flex items-center gap-2 text-[12px] text-white/65">
                ...
                {a.reaction && <span className="text-white/45">— {a.reaction}</span>}
              </li>
            ))}
            {allergies.length > 4 && (
              <li className="text-[11px] text-white/45">+ {allergies.length - 4} more</li>
            )}
          </ul>
        </div>
        <Link
