      ...
      <form id="close-shift-form" onSubmit={submit} className="space-y-4">
        ...
        <div className="rounded-lg border border-neutral-border bg-neutral-surface2 p-4">
          <div className="mb-3 grid grid-cols-2 gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-[0.05em] text-content-muted">Opening float</p>
              <p className="mt-0.5 font-mono text-[14px] text-ink">{naira(session.openingFloat)}</p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-[0.05em] text-content-muted">Cash sales</p>
              <p className="mt-0.5 font-mono text-[14px] text-success">+ {naira(session.summary?.totalCash ?? 0)}</p>
            </div>
          </div>
          <div className="flex items-center justify-between border-t border-dashed border-neutral-border pt-3">
            <span className="inline-flex items-center gap-1.5 text-[12px] text-content-secondary">
              <Wallet size={12} /> Expected in till
            </span>
            <span className="font-mono text-[16px] font-medium text-ink">{naira(expectedCash)}</span>
          </div>
        ...
        {variance != null && variance !== 0 && (
          <div className={`flex items-center justify-between gap-3 rounded-lg border px-3 py-2 ${
            ...
            <div className="flex items-center gap-2">
              <AlertCircle size={14} className={Math.abs(variance) < 100 ? "text-warning" : "text-danger"} />
              <span className="text-[12px] text-content-secondary">Variance</span>
            </div>
        ...
        {variance === 0 && countedCash && (
          <div className="flex items-center gap-2 rounded-lg border border-success/30 bg-success-bg px-3 py-2 text-[12px] text-success">
