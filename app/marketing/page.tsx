  ...
  return (
    ...
    >
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-5">
        {kpis.map((k) => (
          <div key={k.label} className="rounded-xl border border-white/[0.06] bg-cinema-elev p-4">
            <p className="mb-1 text-[11px] uppercase tracking-[0.05em] text-white/65">{k.label}</p>
            <p className="font-mono text-[22px] font-bold leading-none text-white">{k.kpi ? fmtKpi(k.kpi.value) : "—"}</p>
            {k.kpi?.delta && (
              <p className={`mt-2.5 font-mono text-[11px] ${toneText[k.kpi.tone] ?? "text-white/45"}`}>{k.kpi.delta}</p>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.5fr_1fr]">
        ...
        <div className="space-y-6">
          {/* Upcoming posts */}
          <section className="overflow-hidden rounded-xl border border-white/[0.06] bg-cinema-elev">
            <div className="flex items-center justify-between border-b border-white/[0.06] p-5">
              <h3 className="text-[15px] font-medium text-white">Upcoming Posts</h3>
              <Link href="/marketing/social" className="text-[13px] font-medium text-primary hover:underline">View calendar →</Link>
            </div>
            <div className="space-y-2 p-4">
              {upcoming.length > 0 ? (
                upcoming.map((p) => (
                  <Link key={p.id} href="/marketing/social" className="flex items-center gap-3 rounded-lg p-2 transition-colors hover:bg-white/[0.02]">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary/[0.08] text-primary"><CalendarDays size={20} /></span>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[14px] font-medium text-white">{p.title || p.content || "Untitled post"}</p>
                      <p className="mt-0.5 text-[12px] text-white/45">{fmtWhen(p.scheduledAt)}{p.platform ? ` · ${p.platform}` : ""}</p>
                    </div>
                    <span className="shrink-0 rounded-full bg-white/[0.02] px-2.5 py-1 text-[11px] capitalize text-white/65">{p.status}</span>
                  </Link>
                ))
              ) : (
                <p className="px-2 py-6 text-center text-[13px] text-white/65">No upcoming posts. Schedule one to fill your calendar.</p>
              )}
          ...
          <section className="overflow-hidden rounded-xl border border-white/[0.06] bg-cinema-elev">
            <div className="border-b border-white/[0.06] p-5">
              <h3 className="text-[15px] font-medium text-white">Active Campaigns <span className="font-normal text-white/45">(Email and SMS)</span></h3>
            </div>
            <div className="space-y-4 p-5">
              {activeCampaigns.length > 0 ? (
                activeCampaigns.map((c) => (
                  <div key={c.id} className="rounded-lg border border-white/[0.06] bg-cinema-elev p-4">
                    <div className="mb-4 flex items-start justify-between">
                      <div>
                        <h4 className="text-[15px] font-semibold text-white">{c.name}</h4>
                        <span className="mt-1 inline-block rounded bg-primary/[0.08] px-2 py-0.5 text-[11px] font-medium capitalize text-primary">{c.type}</span>
                      </div>
                      <span className="flex items-center gap-1.5 text-[12px] capitalize text-emerald-300"><span className="h-2 w-2 rounded-full bg-success" /> {c.status}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-[11px] uppercase tracking-[0.05em] text-white/45">{c.type?.toLowerCase() === "sms" ? "Delivered" : "Sent"}</p>
                        <p className="font-mono text-[18px] text-white">{(c.type?.toLowerCase() === "sms" ? c.delivered : c.sent).toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-[11px] uppercase tracking-[0.05em] text-white/45">{c.type?.toLowerCase() === "sms" ? "Click rate" : "Open rate"}</p>
                        <p className="font-mono text-[18px] text-white">{((c.type?.toLowerCase() === "sms" ? c.clickRate : c.openRate) || 0).toFixed(1)}%</p>
                      </div>
                    </div>
                  </div>
                ))
              ) : (
                <p className="py-6 text-center text-[13px] text-white/65">No active campaigns yet.</p>
              )}
            </div>
          </section>
        </div>
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-xl border border-white/[0.06] bg-cinema-elev p-6">
            <h3 className="mb-6 text-[15px] font-medium text-white">Leads Funnel</h3>
            {stages.length > 0 ? (
              <>
                  ...
                  {stages.map((f, i) => (
                    <div key={f.stage}>
                      <div className="flex items-center gap-3">
                        <div className="h-8 flex-1 rounded-sm bg-white/[0.02]">
                          <div className="h-8 rounded-sm bg-primary" style={{ width: `${Math.max(8, (f.count / maxStage) * 100)}%`, opacity: 1 - i * 0.15 }} />
                        </div>
                        <span className="w-8 text-right font-mono text-[14px] text-white">{f.count}</span>
                      </div>
                      <p className="mt-1 text-[12px] capitalize text-white/65">{f.stage}</p>
                    </div>
                  ))}
                </div>
                {funnel.data?.conversionRate != null && (
                  <div className="flex justify-between border-t border-white/[0.06] pt-4">
                    <span className="text-[14px] text-white/45">Conversion Rate</span>
                    <span className="font-mono text-[14px] font-bold text-white">{funnel.data.conversionRate.toFixed(1)}%</span>
                  </div>
                )}
              </>
            ) : (
              <p className="py-4 text-center text-[13px] text-white/65">No leads yet.</p>
            )}
          </section>
            ...
            <div className="flex flex-col gap-2">
              {QUICK.map((a) => (
                <Link key={a.label} href={a.href} className="group flex items-center gap-3 rounded-lg border border-white/[0.06] px-4 py-3 text-left text-[14px] text-white transition-colors hover:bg-white/[0.02]">
                  ...
                  <ChevronRight size={16} className="text-white/45 opacity-0 transition-opacity group-hover:opacity-100" />
                </Link>
              ))}
            </div>
          </section>
